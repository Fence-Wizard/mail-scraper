...
class Task(Base):
    ...
    assigned_purchaser_email: Mapped[str | None] = mapped_column(String(320), index=True)
    assigned_by_email: Mapped[str | None] = mapped_column(String(320))
    material_in_stock: Mapped[bool | None] = mapped_column(Boolean)
    can_pull_extra: Mapped[bool | None] = mapped_column(Boolean)
    po_provided: Mapped[bool | None] = mapped_column(Boolean)
    prices_valid: Mapped[bool | None] = mapped_column(Boolean)
    all_material_present: Mapped[bool | None] = mapped_column(Boolean)
    vendor_coord_price: Mapped[str | None] = mapped_column(Text)
    vendor_coord_delivery_time: Mapped[str | None] = mapped_column(Text)
    vendor_coord_delivery_location: Mapped[str | None] = mapped_column(Text)
    expected_delivery_date: Mapped[str | None] = mapped_column(DateTime(timezone=True))
    backorder_notes: Mapped[str | None] = mapped_column(Text)
    decision_path: Mapped[list | None] = mapped_column(JSON)
